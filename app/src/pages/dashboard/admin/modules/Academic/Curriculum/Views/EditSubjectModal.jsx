...
const EditSubjectModal = ({ isOpen, onClose, onSuccess, subjectData }) => {
  ...
  // Populate form when subjectData changes
  useEffect(() => {
    if (subjectData) {
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!subjectData?.subjectId && !subjectData?.subject_id) {
      toast.error('ID mata pelajaran tidak ditemukan');
      return;
    }

    setLoading(true);
    try {
      const subjectId = subjectData.subjectId || subjectData.subject_id;
      const response = await api.put(`/subjects/admin/${subjectId}`, {
        subjectName: formData.subjectName.trim(),
        description: formData.description.trim() || null
      });

      toast.success(response.data.message || 'Mata pelajaran berhasil diperbarui');
      ...
      const errorMessage = error.response?.data?.message || 'Gagal memperbarui mata pelajaran';
      ...
      >
        ...
        <div className="bg-gradient-to-r from-purple-600 to-purple-700 px-6 py-4 flex items-center justify-between rounded-t-xl">
          <div className="flex items-center">
            ...
            <h3 className="text-lg font-bold text-white">Edit Mata Pelajaran</h3>
          </div>
          <button
            onClick={handleClose}
            disabled={loading}
